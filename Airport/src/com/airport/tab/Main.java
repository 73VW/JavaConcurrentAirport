...
public class Main
	...
	public static void main(String[] args)
		...
		TabAvion airArr = new TabAvion(nbAvion);
		TabAvion tarmacLand = new TabAvion(nbPisteArr);
		TabAvion tarmacTakeOff = new TabAvion(nbPisteDep);
		TabAvion terminal = new TabAvion(nbPlace);
		TabAvion airDep = new TabAvion(nbAvion);

		for(int i = 0; i < nbAvion; i++)
			{
			AvionTableau avion = new AvionTableau(airportFrame, codePlane[i], airArr, tarmacLand, tarmacTakeOff, terminal, airDep, nbAvion, nbPisteArr, nbPisteDep, nbPlace);
